...
class ToolBar extends HTMLElement {
    ...
    updateMode() {
        ...
        if (mode === 'simple') {
            if (this.togglePanelBtn) this.togglePanelBtn.style.display = 'none';
            if (this.hideTitleBtn) this.hideTitleBtn.style.display = 'none';
            if (this.emailBtn) this.emailBtn.style.display = 'none';
        }
    }
    ...
    async handleRandomVideo() {
        try {
            const response = await fetch('/assets/config/background_video.json');
            if (!response.ok) {
